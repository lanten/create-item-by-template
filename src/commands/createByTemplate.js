      ...
      .then(inputPath => {
        ...
        if (fs.existsSync(folderPathFull))
          return vscode.window.showWarningMessage(
            localize.getLocalize('text.warning.folderExisted', folderPathFull)
          )
        ...
        if (typeof templateObj !== 'object')
          return vscode.window.showErrorMessage(localize.getLocalize('text.error.templateConfig'))

        try {
          mkdirRecursive(folderPath)
        } catch (error) {
          return vscode.window.showErrorMessage(localize.getLocalize('text.error.create.folders', error))
        }

        for (const fileName in templateObj) {
          const templateArr = templateObj[fileName]
          if (templateArr.constructor !== Array)
            return vscode.window.showErrorMessage(localize.getLocalize('text.error.templateConfig'))
          ...
          if (creatRes)
            vscode.window.showErrorMessage(
              localize.getLocalize('text.error.create.files', `${filePath}\n${creatRes}`)
            )
        }
...
templateListPicker.onDidAccept(() => {
  ...
  if (typeof selectItem.fn !== 'function')
    return vscode.window.showErrorMessage(localize.getLocalize('text.error.templateFunction', selectItem.label))
